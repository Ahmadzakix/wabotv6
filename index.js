...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Ahmad Zaki\n' // GANTI NAMA LU
            + 'ORG:OWNER HLX-BOT;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6283873395995:+62 838-7339-4995\n' // GANTI NOMOR LU
            + 'END:VCARD'
// UDAH SEGITU KONSOL KEBAWAH BIARIN AJA
const ngonsol = JSON.parse(fs.readFileSync('./settings/HLXBOT.json'))
const {
    botName,
    ownerName,
    BarBarKey,
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	...
	client.on('credentials-updated', () => {
		fs.writeFileSync('./HLXBOT.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./HLXBOT.json') && client.loadAuthInfo('./HLXBOT.json')
	client.on('connecting', () => {
		start('2', 'HLXBOT Connecting...')
	})
	client.on('open', () => {
		success('2', 'HLXBOT Connected')
	})
...
*/
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 HLX baikk hati`)
				teks = body.slice(8)
				reply('「❗」SEBENTAR GUA NULIS DULU')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${teks}&APIKEY=${XteamKey}`)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 HLXBOT baik hati`)
				teks2 = body.slice(8)
				reply('「❗」SEBENTAR GUA NULIS DULU')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis3?text=${teks2}&APIKEY=${XteamKey}`)
				client.sendMessage(from, buff, image, {quoted: mek, caption: 'Lebih baik nulis sendiri ya kak :*'})
				break
				case 'nulis':
				case 'tulis':
				...
				client.sendMessage(from, ''+tes+'\n\n_By : Ahmad Zaki ._', text, { quoted: mek })
				await limitAdd(sender)
				break				
				case 'ninjalogo':
                  if (isBanned) return reply(ind.baned())
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo Owner & Gans`)
				reply(ind.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}gemboktext 12 02 2021 & Zaki dan Caca`)
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo HLXBOT', text, {quoted: mek})
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apa?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata HLXBOT`
...
case 'neontext':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}neontext HLXBOT`)
neon = body.slice(10)
reply('「❗」WAIT GANS')
...
case 'blackpink':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink HLX`)
pink = body.slice(11)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Ahmad Zaki`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena Zaki baik Bot memberikan ${one}Xp >_<`)
                      }else{
